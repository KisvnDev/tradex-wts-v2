...
import * as config from '../OrderBook/config';
...
import { ActiveOrderStatusDropdown } from './enum';
import {
  CellClickedEvent,
  GridApi,
  GridReadyEvent,
...
import { queryOrderBook } from '../OrderBook/actions';
import {
  subscribeOrderMatch,
  unsubscribeOrderMatch,
} from 'redux/global-actions';
import { withErrorBoundary } from 'react-error-boundary';
import CancelMultiOrderModal from '../OrderBook/CancelMultiOrder';
import OrderInfo from '../OrderBook/OrderInfo';

interface IOrderBookProps
  extends React.ClassAttributes<ActiveOderComponent>,
    WithNamespaces,
    RouteComponentProps {
...
interface IOrderBookState {
  ...
  readonly orderStatus: ActiveOrderStatus;
...
class ActiveOderComponent extends React.Component<
  IOrderBookProps,
  IOrderBookState
> {
  ...
  constructor(props: IOrderBookProps) {
    ...
    this.state = {
      ...
      orderStatus: ActiveOrderStatus.ALL,
  ...
  render() {
    ...
    const rowData = orderBook.data.filter(
      (order) =>
        order.orderTime &&
        (order.orderStatus === OrderStatusResponse.QUEUE ||
          order.orderStatus === OrderStatusResponse.STOP_ORDER_INACTIVE ||
          order.orderStatus === OrderStatusResponse.STOP_ORDER_READY ||
          order.orderStatus === OrderStatusResponse.PENDINGAPPROVAL ||
          order.orderStatus === OrderStatusResponse.READY_TO_SEND ||
          order.orderStatus === OrderStatusResponse.WAITING_MODIFY ||
          order.orderStatus === OrderStatusResponse.PARTIALLY_EXPIRED ||
          order.orderStatus === OrderStatusResponse.PARTIALLY_FILLED ||
          order.orderStatus === OrderStatusResponse.INACTIVE_ORDER)
    );
    const lastRowData = rowData.reduce(
      (val, curr) => ({
        totalCell: t('TOTAL 2'),
    ...
    const RenderItem = (
      <div className={styles.OrderBookTab}>
        <div className={styles.OrderBookFilter}>
          ...
          <div hidden className={styles.OrderBookDropdown}>
            <p>{t('Status')}</p>
            <Dropdown
    ...
    return (
      <div className={styles.OrderBook}>
        <TabTable
          data={orderBookTab(AccountRoutes.ACTIVE_ORDER, RenderItem)}
  ...
  private onSelectOrderStatus = (data: ActiveOrderStatus) => {
...
const ActiveOder = withRouter(
  withErrorBoundary(
    withNamespaces('common')(
      connect(mapStateToProps, mapDispatchToProps)(ActiveOderComponent)
    ),
    Fallback,
    handleError
  )
